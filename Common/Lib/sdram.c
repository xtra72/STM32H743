...
#define SDRAM_BLOCK_SIZE_MIN	( ( size_t ) ( 32 ) )

/* Assumes 8bit bytes! */
#define SDRAM_BITS_PER_BYTE		( ( size_t ) 8 )
 ...
 */
static void SDRAM_insertBlockIntoFreeList( BlockLink_t *pxBlockToInsert );
		...
		if( ( xWantedSize & xBlockAllocatedBit ) == 0 )
		{
			...
			if( ( xWantedSize > 0 ) && ( xWantedSize <= xFreeBytesRemaining ) )
			{
				...
				if( pxBlock != pxEnd )
				{
					...
					two. */
					if( ( pxBlock->xBlockSize - xWantedSize ) > SDRAM_BLOCK_SIZE_MIN )
					{
						...
						SDRAM_insertBlockIntoFreeList( pxNewBlockLink );
					}
					else
					{
						mtCOVERAGE_TEST_MARKER();
		...
		if( ( pxLink->xBlockSize & xBlockAllocatedBit ) != 0 )
		{
			if( pxLink->pxNextFreeBlock == NULL )
			{
				...
				vTaskSuspendAll();
				{
					...
					SDRAM_insertBlockIntoFreeList( ( ( BlockLink_t * ) pxLink ) );
				}
				( void ) xTaskResumeAll();
			}
			else
...
void SDRAM_heapInit( void* heapStart, uint32_t heapSize)
{
	...
	uxAddress = ( size_t ) heapStart;

	if( ( uxAddress & portBYTE_ALIGNMENT_MASK ) != 0 )
	{
		uxAddress += ( portBYTE_ALIGNMENT - 1 );
		uxAddress &= ~( ( size_t ) portBYTE_ALIGNMENT_MASK );
		xTotalHeapSize -= uxAddress - ( size_t ) heapStart;
	}
	...
	xBlockAllocatedBit = ( ( size_t ) 1 ) << ( ( sizeof( size_t ) * SDRAM_BITS_PER_BYTE ) - 1 );
}
/*-----------------------------------------------------------*/
